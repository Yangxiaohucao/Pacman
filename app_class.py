...
############## Start Functions######################################################
    ...
    def start_events(self):
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE:
                self.state ='playing'
                self.mixer.music.load('pacman_beginning.wav')
    ...
    def start_draw(self):
        ...
        self.draw_text('PUSH SPACE TO START',self.screen,[WIDTH//2,HEIGHT//2-50], 
        START_TEXT_SIZE,(170,132,58),START_FONT,centered=True)
        self.draw_text('YangXiaohuCao',self.screen,[WIDTH//2,HEIGHT//2-30], 
        START_TEXT_SIZE,(166,166,50),START_FONT,centered=True)

        self.draw_text('Shu Miao',self.screen,[WIDTH//2,HEIGHT//2-10], 
        START_TEXT_SIZE,(166,166,50),START_FONT,centered=True)

        self.draw_text('Jerry Li',self.screen,[WIDTH//2,HEIGHT//2+10], 
        START_TEXT_SIZE,(166,166,50),START_FONT,centered=True)
        ...
        self.draw_text('PAC MAN',self.screen,[WIDTH//2,HEIGHT//2-200], 
        START_TEXT_SIZE,(255,255,255),START_FONT,centered=True)
        self.draw_text('Year2021 3815ICT',self.screen,[WIDTH//2,HEIGHT//2-100], 
        START_TEXT_SIZE,(255,255,255),START_FONT,centered=True)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LEFT:
                    ...
                    self.mixer.music.load('pacman_chomp.wav')
                    self.mixer.music.play()
                ...
                if event.key == pygame.K_ESCAPE:
                    result = ctypes.windll.user32.MessageBoxW(0, "You Sure you want to Exit?", "Existing Game", 1)
                    if result ==1:
                     self.running = False
                     pygame.QUIT
                    else: 
